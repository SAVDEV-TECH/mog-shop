...
export default function DashboardPage() {
  ...
  const setupRealtimeListeners = () => {
    ...
    try {
      // Orders listener with error handling
  ...
  const uploadImage = async (file: File): Promise<string> => {
    try {
      ...
      if (!cloudName) {
        throw new Error('Cloudinary cloud name not configured. Please add NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME to your .env.local file');
      }
